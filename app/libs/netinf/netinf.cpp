...
int main(int argc, char* argv[]) {
  ...
  // TFIn FIn(InFNm);
  // take input from memory instead of a predefined file
  TMIn SIn(pStr);
  NIB.LoadCascadesTxt(SIn, Model, alpha);
  ...
  if (ComputeInfo){
    ...
    std::cout << NIB.DumpEdgeInfoStr(1024).GetCStr();
  }
